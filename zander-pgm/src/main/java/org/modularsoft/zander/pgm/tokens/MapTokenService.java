...
/**
 * Validates and applies Map Token requests. Requests only ever affect the NEXT
 * match/vote and never interrupt a running match. Failed requests are reported
 * back so zander-web can refund tokens when {@code refundIfFailed} is enabled.
 */
public class MapTokenService {
    ...
    public int pendingCount() {
        return 0; // requests are applied inline; queue lives in zander-web
    }
