...
/**
 * Attempts to set a member's nickname. Returns true on success, false on failure.
 */
...
export async function checkAndReportNickname(member, reportChannelId, trigger = "Nickname Change") {
  try {
    ...
    let enforced = false;
    if (enforcedNickname) {
      enforced = await setMemberNickname(member, enforcedNickname);
    }
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: "Action", value: enforced ? `Nickname set to **${enforcedNickname}**` : "Failed to update nickname (missing permissions?)", inline: false }
      )
      .setTimestamp();
