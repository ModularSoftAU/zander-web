  ...
  async function fetchProfile(uuid) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error(`[nameMcLookup] Non-OK response fetching profile: HTTP ${response.status}`);
      ...
      if (previousNames === undefined) {
        ...
        console.error("[nameMcLookup] Profile page loaded but name-history container not found (markup may have changed)");
        return { status: "unavailable" };
      }

      return { status: "found", previousNames };
    } catch (err) {
      console.error("[nameMcLookup] Failed to fetch profile:", err?.message ?? err);
      return { status: "unavailable" };
    } finally {
