...
/**
 * Normalises a uuid to the 32-char lowercase hex (no dashes) format used by
 * the `userRanks` cross-database view, which stores uuid as
 * `LOWER(HEX(lp_binary_uuid))`. `users.uuid` is stored dashed (36-char), so
 * callers passing that format need this before querying `userRanks`.
 */
