...
function personNameOf(node) {
  ...
  return firstText(
    ...
    node["display-name"],
    node.displayName,
  ) || node?.["@_name"] || node?.["@_username"] || node?.["@_nickname"] || node?.["@_display-name"] || node?.["@_displayName"] || null;
}
...
function extractAuthors(mapNode) {
  // PGM's official schema nests authors under <credits><author>...</author></credits>;
  // some maps instead use a bare <authors><author> or top-level <author> tag. Check all.
  const authorsNode = mapNode.credits?.author ?? mapNode.authors?.author ?? mapNode.author;
  return toArray(authorsNode)
    .map((a) => personNameOf(a))
    .filter(Boolean);
}

function extractContributors(mapNode) {
  const contribNode = mapNode.credits?.contributor ?? mapNode.contributors?.contributor ?? mapNode.contributor;
  return toArray(contribNode)
    .map((c) => personNameOf(c))
    .filter(Boolean);
}
