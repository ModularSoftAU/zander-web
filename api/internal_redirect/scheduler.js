...
export default function schedulerRedirectRoute(app, config, lang) {
  const baseEndpoint = "/redirect/scheduler";

  app.post(baseEndpoint + "/discord/create", async function (req, res) {
    if (!hasPermission("zander.web.scheduler", req, res)) return;
    ...
    res.redirect(`${process.env.siteAddress}/dashboard/scheduler`);
